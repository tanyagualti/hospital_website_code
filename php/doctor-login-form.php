...
<div class="col-md-4" style="position:absolute;top:60%;left: 50%;transform: translate(-50%,-50%);">
   ...
   <div style="background: linear-gradient(to right, #1d86df 0%, #39b49a 100%);"class="pricing-table-header grd1">
    <h2 style="text-align: center;">doctors DashBoard</h2>
      <h3 style="text-align:center;">LifeCare login</h3>
                                 </div>
   ...
   <div style="margin:0px;"class="pricing-table-features">
    <form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post">
      ...
      <span class= "material-symbols-outlined" id="ic"> person</span>
       <input required class="input" style=" width: 100%;" type="text" name="name" placeholder="doctor_name">
 ...
 <a style="color:red;" href="forgot-password.php?id=<?php if (isset($_SESSION['id'])) echo $_SESSION['id']; ?>">forgot password?</a>
    </div>  
...
$sql="select d_id,d_name,d_password from doc where d_name='$name' and d_password='$password'";


    if($result=mysqli_query($conn,$sql)){
        if(mysqli_num_rows($result)>0){
        while($rows=mysqli_fetch_assoc($result)){
            $_SESSION['id']=$rows['d_id'];
             $_SESSION['name']=$rows['d_name'];

             ?>
             <script type="text/javascript">
                 
                 window.location.href='http://localhost/hospital/lifecare/php/doctor-login-insert.php';
             </script>
...
?>
         ...
         <h6 style="text-align:center;"> <a href="../php/patient-registration-form.php"> Create Account!!!!</a></h6>
